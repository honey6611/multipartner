            ...
            _.forOwn(value,  function(value1, key1) { // each quote ex: 1A0E62FBDAEAB41F3A60969DE125C153
                ...
                _.forOwn(value1,  function(value2, key2) {
                    ...
                    _.forOwn(value2,  function(value3, key3) {
                        ...
                        var InserQuery=`INSERT INTO APISearchData (sessionid,PriceID,SearchTimeStamp,Pricing,Distance,duration,UnitID,Vehicle,OccupancyFrom,OccupancyTo,CurID,TotalCostPriceSingle,TotalCostPriceReturn,Min_Stops,Max_Stops,NumUnits,company_logo,Transferflag,VehicleImage,company,TransferType,company_search_Resultid,supplierid,Company_sessionid)
                        values ('${sessionid}','${PriceID}','${SearchTimeStamp}','${Pricing}','${distance}','${duration}','${UnitID}','${Vehiclename}','${OccupancyFrom}','${OccupancyTo}','${CurID}','${TotalCostPriceSingle}','${TotalCostPriceReturn}','${Min_Stops}','${Max_Stops}','${NumUnits}','${company_logo}','${Transferflag}','${VehicleImage}','${company_name}',1,${company_search_Resultid},${setting.provider.taxicode.id},${Company_sessionid})`
                        //console.log(InserQuery)
                        //let result1 = sql.request()
                        let result1 = sql.query `INSERT INTO APISearchData (sessionid,PriceID,SearchTimeStamp,Pricing,Distance,duration,UnitID,Vehicle,OccupancyFrom,OccupancyTo,CurID,TotalCostPriceSingle,TotalCostPriceReturn,Min_Stops,Max_Stops,NumUnits,company_logo,Transferflag,VehicleImage,company,TransferType,company_search_Resultid,supplierid,Company_sessionid)
                        values (${sessionid},${PriceID},getdate(),${Pricing},${distance},${duration},${UnitID},${Vehiclename},${OccupancyFrom},${OccupancyTo},${CurID},${TotalCostPriceSingle},${TotalCostPriceReturn},${Min_Stops},${Max_Stops},${NumUnits},${company_logo},${Transferflag},${VehicleImage},${company_name},1,${company_search_Resultid},${setting.provider.taxicode.id},${Company_sessionid})`
                    })
                    }   
                })
            })
